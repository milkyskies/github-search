...
- 失敗は例外ではなく型付きの値です。GitHub データ層は `Result` ユニオンを返すため、`ZodError` や fetch の throw が境界の外に漏れません。
- あらゆる失敗は名前付きの種別（レート制限・ネットワーク・タイムアウト・不正なクエリ・not-found・パース）に分類し、汎用的なメッセージではなく原因に応じたメッセージを表示します。
- 再試行は実際に成功し得る場合（レート制限やネットワークの一時的な不調といった一時的な種別）にのみ提示し、不正なクエリや不正な形式のレスポンスのような恒久的な失敗には出しません。この仕組みは、検索のインラインエラー・追加読み込みの失敗・詳細ページで共通です。
- 各エラー分類（`HttpError` と `GithubError` の各種別）にはユニットテストがあり、対応を誤るとビルドが落ちます。
...
- あらゆる失敗は境界で分類し（レート制限・ネットワーク・タイムアウト・不正なクエリ・not-found・パース）、原因に応じたメッセージを表示します。再試行は実際に成功し得る一時的なケース（レート制限やネットワークの一時的な不調）にのみ提示し、不正なクエリや不正な形式のレスポンスのような恒久的な失敗には出しません。
...
- 正常系と異常系の両方の振る舞いを [`docs/test/`](docs/test/) の Gherkin 形式の仕様ドキュメント（[search](docs/test/search.md)・[detail](docs/test/detail.md)）に、安定した `SEARCH-NNN` / `DETAIL-NNN` コードと、それを自動化するテストへのリンク付きで記述しています。
